...
Page({
  ...
  getTopicDetail() {
    ...
    util.request(api.TopicDetail, { id: that.data.topicId}).then(function (res) {
      if (res.errno === 0) {
        if (res.data) {
          WxParse.wxParse('content', 'html', res.data.topic.content, that, 5);
...
*/
  ...
  _sharesuccess: function () {
    let that = this;
    let sharepic = that.data.item.headPic
 ...
 */
  ...
  _edit: function(){
    try {
      wx.setStorageSync('editarc', { data: this.data.item});
    } catch (e) {
    }
    this.shares.hideModal()
    wx.navigateTo({
      url: '/pages/publish/arcticle/arcticle'
    })
  },
  goIndex: function () {
    wx.reLaunch({
