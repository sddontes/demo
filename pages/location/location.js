...
Page({
  ...
  onLoad: function (options) {
    wx.createSelectorQuery().selectAll('.city-item-A-Z').fields({ dataset: true, size: true, rect: true }, res => {
      this.setData({ titleObj: res })
    }).exec();
  ...
  onPageScroll: function (e) { // 获取滚动条当前位置
    ...
    let titleObj = this.data.titleObj;
    for (let i = 0; i < titleObj.length; i++) {
      if (titleObj[i].top > e.scrollTop) {
        // console.log(titleObj[i - 1].dataset.cityname);
        this.setData({ fixedTitle: titleObj[i - 1].dataset.cityname });
        return;
      }
    }
    wx.createSelectorQuery().selectAll('.city-item-A-Z').fields({ dataset: true, size: true, rect: true }, function (res) {
      //console.log(e.scrollTop,res.top);
      for (let i = 0; i < res.length; i++) {
        if (res[i].top < e.scrollTop) {
          //console.log(res[i].dataset.cityname);
          //this.setData({ fixedTitle: res[i].dataset.cityname });
          //return;
        }
      }
  ...
  citySelected: function (e) {
    ...
    try {
      wx.setStorageSync('setLocation',cityNameTemp);
    } catch (e) {
    }
